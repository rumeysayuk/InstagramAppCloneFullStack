...
import {signIn, signUp, getUser, addUser} from "../../actions/auths";
...
const Auth = () => {
    ...
    const [getUser, addUser] = useState(false)
    const [isGoogleSuccess, setGoogleSuccess] = useState(null);
    ...
    const matchPassword = () => {
        {
            if (form.password === form.confirmPassword) {
                ...
                (alertify.error("Şifreler eşleştirilemedi", "Şifre hatası"))
            }
        }
    }
    ...
    return (
        {
                    ...
                    <Paper className={classes.paper} elevation={6}>
                        ...
                        <form className={classes.form} onSubmit={handleSubmit}>
                            <Grid container spacing={2}>
                                {isSignUp && (
                                    <>
                                        <Input name={"firstName"} label={"İsminiz"} handleChange={handleChange}
                                               autoFocus half/>
                                        <Input name={"lastName"} label={"Soyisminiz"} handleChange={handleChange} half/>
                                    </>
                                )}
                                <Input name={"email"} label={"Email Adresiniz"} handleChange={handleChange}
                                       type={"email"}/>
                            ...
                            <Typography component={"h1"} variant={"h5"} align={"center"}
                                         style={{marginBottom: "15px"}}>Ya da</Typography>
                            <GoogleLogin
                                clientId={"840807562256-5ovit9p0l6bprfvsibqutv2947duuu3b.apps.googleusercontent.com"}
                                render={(renderProps) => (
            ...
            (
                <Input name={"userName"} label={"Kullanıcı adınız"} handleChange={handleChange}
                       autoFocus half/>,
                    <Input name={"password"} label={"Şifre"} handleChange={handleChange}
                           type={showPassword ? "text" : "password"}
                           handleShowPassword={() => setShowPassword(!showPassword)}/>,
